...
public class DispatcherServlet extends FrameworkServlet {
	...
	static {
		...
		/**
		 * 从属性文件中加载默认策略实现
		 * 就是从 DEFAULT_STRATEGIES_PATH 这个文件当中拿出所有的配置，共有8个
	...
	protected void initStrategies(ApplicationContext context) {
		// 上传文件的bean，这里面就写清楚了处理上传文件的bean的beanName为什么为 multipartResolver
		initMultipartResolver(context);
		// 国际化的
		initLocaleResolver(context);
		// 前端的主题样式，一般不用
		initThemeResolver(context);
		// 初始化HandlerMappings，非常重要
	...
	private void initMultipartResolver(ApplicationContext context) {
		try {
			// MULTIPART_RESOLVER_BEAN_NAME 固定为 multipartResolver
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// 通过 HandlerMapping 类确定 HandlerExecutionChain对象类型
		...
		try {
			...
			try {
				...
				/**
				 * 确定当前请求的处理程序。推荐 controller 和 HandlerExecutionChain 的类型
				 * 存在不同的映射中，共有两种(BeanNameUrlHandlerMapping、RequestMappingHandlerMapping)。
				 * 这里也证明了，为什么springboot中直接访问 /index.html，访问不到。因为上面两种映射中，
				 * 都找不到对应的处理类型
				 *
				 * controller有三种类型:
				 *
				...
				/**
				 * 如果 HandlerExecutionChain 是一个bean，mappedHandler.getHandler()返回一个bean对象
				 * 如果 HandlerExecutionChain 是一个方法，mappedHandler.getHandler()返回一个method对象
				 */
				HandlerAdapter ha = getHandlerAdapter(mappedHandler.getHandler());
				// 上面的完成后，到这里spring才知道怎么反射调用
	...
	protected HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		// HandlerMapping 在本类的static块进行初始化的
		if (this.handlerMappings != null) {
			// 从两种不同的 HandlerMapping中找出合适的
			for (HandlerMapping mapping : this.handlerMappings) {
				// 把请求传过去，来确定。怎么得到 HandlerExecutionChain，与自己实现的代码有关系
				// 会从 HandlerMappings中去寻找
