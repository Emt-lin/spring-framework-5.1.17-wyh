...
class ConfigurationClassEnhancer {
	...
	public Class<?> enhance(Class<?> configClass, @Nullable ClassLoader classLoader) {
		// 判断是否被代理过，通过是否实现了EnhancedConfiguration 接口判断
		...
		// 如果没有被代理，就cglib代理，进入查看newEnhancer
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		 ...
		 * 主要为生成的CGLIB类添加成员变量 $$beanFactory,
		 * 同时基于接口 EnhancedConfiguration 的父接口 BeanFactoryAware 中的setFactory方法，设置此变量的值
		 * 为当前Context中的beanFactory，这样就可以通过beanFactory获得对象
		 * 该beanFactory的作用是在this调用时拦截该调用，并直接从beanFactory获得目标对象bean进行返回
		 */
		enhancer.setStrategy(new BeanFactoryAwareGeneratorStrategy(classLoader));
		// CALLBACK_FILTER 里面存在方法拦截器，拦截器保证了spring的单例性，同时也给$$beanFactory注入值
	...
	private static class BeanMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object enhancedConfigInstance, Method beanMethod, Object[] beanMethodArgs,
			...
			// 因为spring严格要求的是：为单例bean时，如果已经存在了bean，必须从缓存中拿到
			...
			// 必须确保从内部调用FactoryBean语义 和 外部(XML中)调用的语义一致
			...
			// 判断当前执行的方法 和 调用的方法是不是同一个方法（就是在一个方法内，调用另一个方法）
			// 如果在一个方法的内部调用一个方法，外面的方法是 cglibMethodProxy，内部调用的方法是 beanMethod
			/*
			 * 如果为true，直接调用父类的方法。cglibMethodProxy.invokeSuper()
			 */
